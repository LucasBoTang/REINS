...
class TestRelaxationNodeInit:
    ...
    def test_multi_output_keys(self):
        net = nn.Linear(4, 6)
        rel = RelaxationNode(net, ["b"], ["x", "y"], sizes=[3, 3])
    ...
    def test_name(self):
        net = nn.Linear(4, 3)
        rel = RelaxationNode(net, ["b"], ["x"], name="my_relaxation")
...
class TestRelaxationNodeForward:
    ...
    def test_uneven_split(self):
        net = nn.Linear(4, 7)
        rel = RelaxationNode(net, ["b"], ["x", "y"], sizes=[5, 2])
        data = {"b": torch.randn(8, 4)}
        result = rel(data)
    ...
    def test_split_correctness(self):
        ...
        net = nn.Linear(4, 6)
        rel = RelaxationNode(net, ["b"], ["x", "y"], sizes=[4, 2])
        b = torch.randn(8, 4)
        full_output = net(b)
        data = {"b": b}
        result = rel(data)
