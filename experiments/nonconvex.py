...
def build_loss(x, b, d, num_var, num_ineq, penalty_weight, sparse=False, device="cpu", relaxed=False):
    """
...
def run_EX(loader_test, config):
    ...
    model = nonconvex(num_var, num_ineq, sparse=config.sparse, timelimit=1000)
    # Init result lists
...
def run_AS(loader_train, loader_test, loader_val, config):
    ...
    b = Variable("b")
    d = Variable("d")
    loss = build_loss(x, b, d, num_var, num_ineq, penalty_weight, sparse=config.sparse, device="cuda")
...
def run_LR(loader_train, loader_test, loader_val, config):
    ...
    b = Variable("b")
    d = Variable("d")
    loss = build_loss(x, b, d, num_var, num_ineq, penalty_weight, sparse=config.sparse, device="cuda", relaxed=True)
